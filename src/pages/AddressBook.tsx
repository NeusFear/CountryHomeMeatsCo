...
import { editVendorDetails, setModal } from "../modals/ModalManager";

export const AddressBook = () => {

  const [search, setSearch] = useState<string>('')
  const regExp = useMemo(() => new RegExp(search.split(' ').map(s => `(?=.*${s})`).join(''), 'i'), [search])
  const vendors = useVendors(Vendor.where('company').regex(regExp) || Vendor.where('primaryContact').regex(regExp), [search])
...
const AddressBookEntry = ({ details, deleteUserDetails }: {details: IVendor, deleteUserDetails: () => void}) => {
    return (
      <div className="cursor-pointer bg-gray-100 hover:shadow-md rounded-lg px-2 py-2 shadow-sm flex flex-row mb-2">
        ...
        <span className="pr-2 flex-1 flex flex-row">{details.primaryContact}</span>
        <span className="pr-2 flex-1 flex flex-row">{details.phoneNumbers}</span>
        <span className="pr-2 flex-1 flex flex-row">{details.emails}</span>
